...
s = rq.session()
script = os.path.join(base_path, 'download_pics.py')
...
def generate_md(index, link, multiple_or_not=False):
    ...
    def download_pics(container, title):
        temp_file = 'temp_data.pl'
        container.insert(0,title)
        with open(temp_file, 'wb') as f:
            pickle.dump(container, f)
        os.popen("python %s %s"%(script, temp_file))
    ...
    try:
        print('获取文章: %s'% index)
        # new_base = os.getcwd()
        response = s.get(url=link, headers=headers, timeout=30)
